  ...
  try {
    ...
    function reset(track_info_msg, queue_info_msg) {
      let embed2 = new MessageEmbed()
        ...
        .setTitle("Infinite | Music Queue")
        .setDescription(`Empty\nJoin a voice channel and queue songs by name or url in here.`)
      let embed3 = new MessageEmbed()
        .setColor(ee.color)
        .setFooter(ee.footertext, ee.footericon)
        .setTitle("Infinite | Currently no song is playing!")
        .setDescription(`> Join a voice channel and enter a song name or url to play.\n\n[Invite Infinite](https://discord.com/api/oauth2/authorize?client_id=891427881593503766&permissions=8&scope=bot)`)
        .setImage("https://cdn.discordapp.com/attachments/509104275121635328/898404347203977256/pngwing.com.png")
    ...
    function QueueEmbed(client, player) {
      ...
      const embed = new MessageEmbed().setAuthor(`Infinite | Music Queue`);
      const multiple = 15;
      const page = 1;
      const end = page * multiple;
      const start = end - multiple;
      const tracks = queue.slice(start, end);
      if (queue.current) embed.addField("**1) CURRENT TRACK**", `${queue.current.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} [${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]}]\nby: ${queue.current.requester}`);
...
async function edit_request_message_queue_info(client, player) {
  try {
    ...
    const start = end - multiple;
    const tracks = queue.slice(start, end);
    if (queue.current) embed.addField("**1) CURRENT TRACK**", `${queue.current.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} [${queue.current.isStream ? "LIVE STREAM" : format(queue.current.duration).split(" | ")[0]}]\nby: ${queue.current.requester}`);
    if (!tracks.length) embed.setDescription(`No tracks in ${page > 1 ? `page ${page}` : "the queue"}.`);
    else embed.setDescription(tracks.map((track, i) => `**${start + ++i})** ${track.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} [${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]}]\nby: ${track.requester}`).join("\n"));
    embed.setColor(ee.color);
